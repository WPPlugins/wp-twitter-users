...
Description: Create formatted lists of Twitter users in badges using a simple shortcode. For example for follow friday pages.
Contributors: 0xtc
Author: Tanin Ehrami
...
if (!class_exists("wpFollowFriday")) {
	class wpFollowFriday {
		...
		function getAdminOptions() {
			$wpFollowFridayAdminOptions = array('template' => 'profile.tidy','CustomTemplate'=>null,'wpQuoteTw3rdSRV'=>'awesome');
		...
		function printAdminPage() {
			...
			if (isset($_POST['update_wpFollowFridaySettings'])) {
				...
				if ('OMG_OMG_AWESOME'== $_POST['wpQuoteTweetUser3rdPartyImageSRV']){
					$ffrOptions['wpQuoteTw3rdSRV']= 'awesome';
				} else {
					$ffrOptions['wpQuoteTw3rdSRV']= 'not_so_awesome';
				}
						...
						<div id="post-body-content">';
							...
							<div id="templateselect" class="stuffbox" style="background:#fff;">
								...
								<div class="inside">
									...
									if ('awesome' == $ffrOptions['wpQuoteTw3rdSRV']){
		...
		function wp_just_link_to_user($att,$content=null){
			...
			foreach ($att as $twitterUsername){
				...
				$twitterUsername = str_replace('https://twitter.com/','',$twitterUsername);
				if ( (!in_array($twitterUsername,$exLowerAs)) && (!strstr($twitterUsername,'#')) && ($twitterUsername!='')){
					$r_content  .= ' <a href="http://twitter.com/'.$twitterUsername.'" title="'.$twitterUsername.' on Twitter" rel="external">@'.$twitterUsername.'</a>';
					$exLowerAs[] = $twitterUsername;
				}
			}
			return $r_content;
		...
		function templateUser($userinfo,$WPTUTemplate){
			...
			if ($ffrOptions['wpQuoteTw3rdSRV']=='awesome') {
			...
			$replaceArr = Array(
				...
				'<a href="http://twitter.com/'.$userinfo->screen_name.'" title="'.$userinfo->name.': '.$userinfo->description.'" rel="external" style="color:#'.$userinfo->profile_link_color.';">'. $userinfo->screen_name.'</a>',
